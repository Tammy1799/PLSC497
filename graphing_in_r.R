...
#looking at words the following presidents use the most ofen
...
#reagan to obama inaugural addresses
...
#seeing relative mentions of whale and ahab in moby dick. takes a little cuz it's a long boook
textplot_xray(
...
#absolute looks at absolute number of tokens. how many tokens are there and where do they show up in the text
  textplot_xray(
    kwic(data_corpus_inaugural[50:57], "american"),
    kwic(data_corpus_inaugural[50:57], "people"),
    scale = 'absolute'
...
# g is a graphs. instead of having everything be black and white, each keywords are diff colors
#3. Frequency plots
...
#term is name of features and frequency is numbers associated with each

# Sort by reverse frequency order
topDf$term <- with(topDf, reorder(term, -frequency))

#have to make sure to get rid of stop wordss
...
#row names gives us name of the speech
#use uname to extract the actual numbers
...
#mpg is the data, the next parameter is what data we want to use
#this is looking at in terms of cars, displacement and highway
#no points yet
...
#line to make smooth line graph. better to do smooth over line
#in smooth do se = T
ggplot(mpg, aes(x = displ, y = hwy)) +
  geom_smooth()

# Right column: no y mapping needed!
#each time classes of cars appears in dataset
