...
setwd("/Users/tammystugan/Documents/GitHub/TAD_2021/TAD_2021/TAD_2021/TAD_2021/TAD_2021/R lessons")
set.seed(1234)
...
#take tweets from a given day and hashtagBLM into one given document
# Remove non ASCII characters
blm_tweets_sum$text <- stringi::stri_trans_general(blm_tweets_sum$text, "latin-ascii")
#stringi cleans data from social media. strips out all characters not in ASCII

# Removes solitary letters
blm_tweets_sum$text <- gsub(" [A-z] ", " ", blm_tweets_sum$text)
#lets get of all letters by themselves
...
#remove_twitter is to get rid of @ handles and hashtags

# Topic models
## 2 Selecting K

# Identify an appropriate number of topics (FYI, this function takes a while)
#can take awhile if a lot of rare words
...
#metrics gives how many topics is right number of topics (helps choosing k)
dev.new()
...
#set same seed to get same output
system.time(blm_tm <- LDA(blm_dfm, k = k, method = "Gibbs",  control = list(seed = 1234)))
...
# Per topic per word proabilities matrix (beta). can look at terms most associated with topics. topic 16 is about politics
...
#There are still tweets associated with BLM but now there are politicians and celebrities mixed in liked bernie sanders and beyonce.

#### Do you think that the number of topics is linear in the number of documents??
#I do think that the number of topics s linear in the number of documents. 
