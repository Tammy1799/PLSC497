...
setwd("/Users/tammystugan/Documents/GitHub/TAD_2021/TAD_2021/TAD_2021/TAD_2021/TAD_2021/TAD_2021/TAD_2021/TAD_2021/TAD_2021/TAD_2021/R lessons")
...
#{r}
#if we were to say context = document, it's the same thing as a dfm
#closer the words, are together, affects weight
...
author<-  word_vectors["sports", , drop = FALSE] - 
  word_vectors["baseball", , drop = FALSE] +
  word_vectors["basketball", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(author),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)

author<-  word_vectors["cats", , drop = FALSE] - 
  word_vectors["furry", , drop = FALSE] +
  word_vectors["funny", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(author),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)

author<-  word_vectors["song", , drop = FALSE] - 
  word_vectors["record", , drop = FALSE] +
  word_vectors["break", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(author),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)
